...
rf.fit(x_est, y_est)
mask_forest = rf.feature_importances_.round(2) > 0.05
print(dict(zip(X.columns, mask_forest)))
...
# Se procede a dibujar los componentes para ver que tanta separeción hay entre las clases
# %%
...
# %% [markdown]
#Conocer el numero de compoenentes optimos
...
# %% [markdown]
# Por árboles de decisión, random forest y PCA se eliminan las variables exam_ojos_MENOS DE 1 AÑO y tiem_gafas_MAS DE 2 AÑOS, ya que su factor de importancia es cero 
...
# %% [markdown]
# Ejecución del modelo de clasificación con las caracteristicas seleccionadas luego de realizar la selección y extracción de caracteristicas por medio de análisis descriptivo y ejecución de modelos de ML
modelos_vbles = optometria.drop('astigmatismo', axis = 1)
X_ast = modelos_vbles
...
#%%
#Determinar la óptica profundida para el árbol, donde el error de entrenamiento vs el error de prueba no sea muy diferente
k_plot = []
t_plot = []

for k in range(1,20,1):
    random.seed(1988)
    dt = DecisionTreeClassifier(criterion = 'entropy', max_depth=k, random_state=101)
    dt.fit(x_est_ast,y_est_ast)
    predict = dt.predict(x_test_ast)
    accuracy_test = round(dt.score(x_test_ast,y_test_ast)*100,2)
    accuracy_train = round(dt.score(x_est_ast,y_est_ast)*100,2)
...
#El modelo esta sobreajustado sin importar si se ajusta la profundidad

# %%
#De acuerdo al análisis anterior, se determina la profundidad de 7 como la óptima para ejecución del modelo
random.seed(1988)
arbol_ast = DecisionTreeClassifier(criterion = 'entropy', max_depth = 7,random_state=0)
arbol_ast.fit(x_est_ast, y_est_ast)
y_pred_ast = arbol_ast.predict(x_test_ast)


# %%
print("Accuracy:",metrics.accuracy_score(y_test_ast, y_test_ast))
#El modelo da una exactitud del 100% 

# %% [markdown]
# Confirmación  si hay overfitting
accuracy_test = round(arbol_ast.score(x_test_ast,y_test_ast)*100,2)
accuracy_train = round(arbol_ast.score(x_train_ast,y_train_ast)*100,2)

print('Prueba ', accuracy_test)
print('Entrenamiento', accuracy_train)

#%%
confusionMatriz = confusion_matrix(y_train_ast, y_test_ast)
sn.heatmap(confusionMatriz, cmap="Blues", annot=True)
...
print('Accuracy del clasificador logistico para los datos de prueba: {:.2f}'.format(logreg.score(x_test_ast, y_test_ast)))

#%%
from sklearn.metrics import confusion_matrix
from sklearn.metrics import classification_report
confusion_matrix = confusion_matrix(y_test_ast, y_pred_log)
